...
5. If the response is `needs-input`, ask only for `missing_facts`; do not ask for any fact not listed by the receipt. Do not invent facts from missing artifacts and do not default the path to `full`. 若对 `missing_facts` 的追问需要用户决策，必须通过 `ask_user_question` 结构化提问（禁止自由文本追问），每个 `missing_fact` 对应一个 `question`，`id` 以 `dp-0-` 开头，`header` 为 `DP-0 xxx`，`options` 给出候选，推荐项置首（首项加“推荐”标识），`multi_select:false`。若 `missing_facts ≤3`，可一次性通过单个 `ask_user_question` 的 `questions` 数组批量提问；若 `>3`，则分多轮提问，每轮 ≤3 题，优先询问阻断性事实（workflow、change name、intent），非阻断性事实延后。
...
Only after an explicit workflow path is available, 必须通过 `ask_user_question` 结构化提问（禁止自由文本追问）收集剩余 DP-0 决策：change name and one-sentence intent, known constraints, related optimizations (include or stay focused?), and communication preference (ask per decision or draft for review)，例如调用 `ask_user_question({questions:[{id:"dp-0-change-confirm", header:"DP-0 变更确认", question:"请确认变更名与一句话意图：<name>: <intent>，是否正确？", options:[{label:"确认 (Recommended)", description:"信息正确"}, {label:"需调整", description:"需要修改名称或意图"}], multi_select:false}, {id:"dp-0-scope-confirm", header:"DP-0 范围确认", question:"已知约束：<constraints>，是否包含关联优化？沟通偏好：逐项确认 vs 打包评审？", options:[{label:"聚焦当前范围+逐项确认 (Recommended)", description:"仅做当前需求"}, {label:"包含关联优化", description:"一并处理相关优化"}, {label:"打包草案后评审", description:"先出草案再统一评审"}], multi_select:false}]})`。确认后形成包含这些决策、已解析 `artifact_language`、已持久化 workflow 路径及 recommendation-alignment summary 的 combined summary。Preserve existing scope, constraints, and language entries; never replace them with the path summary alone.

只有当所有 `answers` 的 `selected` 包含“确认”类选项后，才执行后续 4 个 `ssf_state_write`，并把用户选择原样写入 `dp_0_decisions` 的 combined summary：调用 `ssf_state_write`（action: "set", changeDir: "<change-dir>", field: "dp_0_decisions", value: "<combined summary>"）（CLI 等价：`ssf state set <change-dir> dp_0_decisions "<combined summary>" --json`）、调用 `ssf_state_write`（action: "set", changeDir: "<change-dir>", field: "dp_0_result", value: "confirmed"）（CLI 等价：`ssf state set <change-dir> dp_0_result confirmed --json`）、调用 `ssf_state_write`（action: "set", changeDir: "<change-dir>", field: "dp_0_confirmed", value: "true"）（CLI 等价：`ssf state set <change-dir> dp_0_confirmed true --json`）、调用 `ssf_state_write`（action: "set", changeDir: "<change-dir>", field: "dp_0_timestamp", value: "$(date -u +%Y-%m-%dT%H:%M:%SZ)"）（CLI 等价：`ssf state set <change-dir> dp_0_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ) --json`）。
