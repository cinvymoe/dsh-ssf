...
- **dsh-ssf-native-tools (2026-09-01)**: Add 12 native write tools (`ssf_state_write`, `ssf_workflow_write`, `ssf_execution_write`, `ssf_checkpoint`, `ssf_handoff`, `ssf_debug`, `ssf_isolate`, `ssf_finish`, `ssf_inject`, `ssf_sync`, `ssf_audit`, `ssf_runtime`) via `lib/cli-runner.js`; update `packages/dsh-ssf/lib/index.js` SECTION_TEXT Tool-first to "所有 ssf 操作优先结构化 ssf_* 工具（含写工具）；ssf_run 仅用于未覆盖的子命令" and Recommended tool order to cover all write tools grouped by 状态/工作流/执行/协作/生命周期/runtime; update `packages/dsh-ssf/README.md` tool table to 19 `ssf_*` tools (6 read + 12 write + `ssf_run` fallback); `ssf_run` retained for uncovered subcommands.
