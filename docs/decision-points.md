...
本文档集中定义了 dsh-ssf 工作流中所有需要用户明确确认的决策点。每个决策点（Decision Point）都是工作流中的关键门禁，确保用户在自动化流程中始终保持最终决策权。工作流中的 skill 在到达决策点时必须暂停执行、向用户呈现所需信息，并等待明确指令后方可继续。
...
- answers 的 selected 原样写入对应 dp_*_result 字段，guard 校验非空。`answers["dp-N-xxx"]` 的选中值原样写入 `dp_N_result`（如 `dp_0_result`/`dp_1_result`/.../`dp_7_result`），`dsh-ssf_guard` 与状态流转前置校验该字段非空，未记录则阻断流转。
...
- **路径选择协议**：低风险 Quick 与 incident Hotfix 同轮展示 recommendation 后，由用户选择 `tdd`、`new-test` 或 `bounded`，再运行 `dsh-ssf workflow accept --source direct-request --verification <strategy>`；其余路径使用 `show`、补齐 missing facts、`recommend` 和 `select --confirm`。direct receipt 代替短路径 DP。
...
- **关联 skill**：`dsh-ssf:workflow-start`
...
- **关联 skill**：`dsh-ssf:need-explorer`
...
- **关联 skill**：`dsh-ssf:contract-builder`
...
- **所需输入**：已批准的 `execution-contract.md`、项目测试基础设施现状，以及 `dsh-ssf execution recommend` 提供的执行模式证据与建议
- **预期输出**：用户明确选择 `Inline`、`Batch Inline` 或 `SDD` 执行模式，build-executor 据此创建受确认的执行计划。DP-4 不重新选择 DP-0 已确认的 `full`、`hotfix` 或 `tweak` 路径。
- **关联 skill**：`dsh-ssf:build-executor`
...
- **证据门禁**：每次失败修复必须通过 `dsh-ssf debug attempt record` 写入结构化 ledger，包含唯一 attempt id、摘要和 change 目录内的物理证据文件。所有路径都必须先有 current、有效的 execution plan；缺失、陈旧或不匹配的 plan 会被拒绝。Wave Review 的 repair failure 不计入 debugging attempt；重复证据也会被拒绝。
- **预期输出**：至少 3 次不同且证据完整的失败尝试后，用户决定继续调试或放弃；仅 `dsh-ssf debug escalate ... --confirm` 可以持久化 DP-5，通用 `state set` 不可写入 `dp_5_*`。
- **关联 skill**：`dsh-ssf:bug-investigator`
...
- **关联 skill**：`dsh-ssf:release-archivist`
...
| 编号 | 名称 | 关联 Skill | 阶段 |
|------|------|------------|------|
| DP-0 | 设计前确认 | `dsh-ssf:workflow-start` | 入口 |
| DP-1 | 需求确认 | `dsh-ssf:need-explorer` | 探索 |
| DP-2 | 工件审查 | `dsh-ssf:spec-writer` | 规划 |
| DP-3 | 契约批准 | `dsh-ssf:contract-builder` | 桥接 |
| DP-4 | 执行模式选择 | `dsh-ssf:build-executor` | 执行 |
| DP-5 | 调试升级 | `dsh-ssf:bug-investigator` | 执行 |
| DP-6 | 验证失败 | `dsh-ssf:release-archivist` | 收尾 |
| DP-7 | 归档确认 | `dsh-ssf:release-archivist` | 收尾 |
