...
flowchart TD
    ...
    B --> C[检测平台: Cursor / CodeBuddy / Claude / 其他]
...
关键点:九个平台(Claude Code / Cursor / Codex / Copilot / Gemini / OpenCode / WorkBuddy / Trae...)共用同一份 `SKILL.md`,区别只在 hook 注入的输出格式(`additionalContext` vs `hookSpecificOutput`)。
...
- [platform-matrix.md](platform-matrix.md) — 多平台安装面
