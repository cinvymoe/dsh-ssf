...
> **历史归档（已剥离）**：本文档记录 v0.8.0 多平台时期的 skill 重命名对照，当前 DSH-only 发行版仅保留 DSH 插件路径（`packages/dsh-ssf`）与通用 CLI；下文提及的 Claude/Cursor/Copilot/Gemini 等均为历史平台，已剥离，仅作历史参考。
...
### Claude Code（历史平台，已剥离）

- 触发入口从 `/workflow-orchestrator` 改为 `/workflow-start`。（历史平台，已剥离）
- 其他 skill 名称在路由时会自动被 `workflow-start` 引用，通常不需要手动输入。

### Cursor（历史平台，已剥离）

- 重新运行 `node scripts/install-cursor.mjs`（历史平台，已剥离），把 skill 复制到 `.cursor/skills/`。
- `.cursor/rules/phase-guard.mdc` 会同步更新。
- `.cursor/` 是本地生成目录，不需要提交到仓库。

### GitHub Copilot CLI（历史平台，已剥离）

- 通过 root `plugin.json` 重新加载插件。（历史平台，已剥离）
- `plugin.json` 中的 `skills` 字段指向 `skills/` 目录，目录重命名后无需手动修改 manifest，只需确保插件刷新。

### Gemini CLI（历史平台，已剥离）

- 重新激活插件，读取更新后的 `GEMINI.md`。（历史平台，已剥离）
